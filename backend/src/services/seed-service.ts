...
const ADDITIVE_SCHEMA_COMPATIBILITY_SQL = `
...
function hydrateHospitalState(state: HospitalState, users: UserRecord[]) {
  ...
  return {
    ...
    familyMembers: state.familyMembers ?? fallbackState.familyMembers,
    medicalHistoryEntries: state.medicalHistoryEntries ?? fallbackState.medicalHistoryEntries,
    clinicalAttachments: state.clinicalAttachments ?? fallbackState.clinicalAttachments,
    telemedicineSessions: state.telemedicineSessions ?? fallbackState.telemedicineSessions,
...
export async function initializeDataStore() {
  ...
  try {
    ...
    await query(ADDITIVE_SCHEMA_COMPATIBILITY_SQL);
  } catch (error) {
    throw new Error(
